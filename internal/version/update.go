...
func Update(ctx context.Context, opts UpdateOptions) (*UpdateResult, error) {
	if runtime.GOOS == "windows" {
		return nil, fmt.Errorf("Windows không hỗ trợ tự cập nhật tại chỗ, vui lòng tải bản mới tại https://github.com/%s/releases", opts.Repo)
	}
	if opts.Repo == "" {
...
func replaceExecutable(dst, src string) (string, error) {
	if dst == "" {
		return "", fmt.Errorf("Không thể định vị tệp thực thi hiện tại")
	}
