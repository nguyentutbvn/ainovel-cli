...
func (s *simulationState) refresh(contentW int) {
	...
	case s.err != nil:
		b.WriteString(errStyle.Render("Xử lý hồ sơ mô phỏng văn phong thất bại"))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render("Esc đóng bảng"))
	default:
		b.WriteString(okStyle.Render("Hồ sơ mô phỏng văn phong đã sẵn sàng, các Agent sau sẽ đọc từ novel_context"))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render("Esc đóng bảng"))
	}
...
func renderSimulationModal(width, height int, s *simulationState) string {
	...
	modal := renderPaddedModalFrame(boxW, boxH, "Mô phỏng văn phong", hint, strings.Split(s.viewport.View(), "\n"))
...
func startSimulate(rt *host.Host, reqID int, args []string, width, height int) (*simulationState, tea.Cmd, error) {
	...
	state := newSimulationState(reqID, "Tạo hồ sơ mô phỏng văn phong", "./simulate", width, height, cancel)
...
func startImportSimulation(rt *host.Host, reqID int, args []string, width, height int) (*simulationState, tea.Cmd, error) {
	...
	state := newSimulationState(reqID, "Nhập hồ sơ mô phỏng văn phong", args[0], width, height, cancel)
	return state, listenSimulationEvent(reqID, ch), nil
}
